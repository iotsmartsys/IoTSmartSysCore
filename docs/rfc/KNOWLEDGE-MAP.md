...
**Última atualização:** 12/09/2026 (BLE Control 0.2: código e builds, aceite pendente)
...
| Transporte BLE de comandos V1 | `docs/specs/BLE-COMMAND-TRANSPORT.md` | Draft 0.2 — análise Ready | In Progress; código construído no ESP32 clássico, aceite operacional pendente (`EKOM-CHG-0026`) |
...
| ADC1 por modelo de ESP32 | `docs/specs/ESP32-ADC1-SUPPORT.md` | Active 0.2 — análise Ready | Implemented / Done; encerrada e integrada à main por ordem do Arquiteto; limitação H2 preservada (`EKOM-CHG-0025`) |
...
| BLE Control | In Progress | `docs/specs/BLE-COMMAND-TRANSPORT.md` | Canal/dispatcher 0.2, Service Data, Auth e vínculo privado; builds padrão/habilitado aprovados; hardware pendente |
...
| Testes | Inventoried | `test/`, `configs/esp32s3-test.ini` | As 18 suítes existentes em 01/08/2026 permanecem sob a quarentena nominal de `BCS-DEC-007`; os testes de capacidade compilam nos perfis 8 e 12 sem upload, mas seus casos permanecem `Not Executed` (`EKOM-CHG-0022`) |
| Exemplos executáveis | Specified | `src/ExecutableExampleRunner.cpp`, `examples/executable/`, `configs/executable_examples.ini` | `screen_console` validado em hardware em `EKM-CHG-0042`; `current_sensor` validado em hardware em `EKM-CHG-0052`; `voltage_sensor` validado em hardware por decisão do Arquiteto em `EKOM-CHG-0004`; `environment_ntc` validado em hardware por decisão do Arquiteto em `EKOM-CHG-0007`; `fan` validado em hardware por decisão do Arquiteto em `EKOM-CHG-0010`; `power_energy` validado e concluído (`EKOM-CHG-0016`); `ina3221_voltage_current` foi validado em hardware (`EKOM-CHG-0021`); `mcb01_solar_controller` foi integrado à `main` com nove capabilities; validação física `Not Executed` aceita pelo Arquiteto (`EKOM-CHG-0022`) |

### 2.2 Fronteira de composição de potência e energia

`IOTSSC-POWER-ENERGY-CAPABILITY@0.4` propõe que
`PowerEnergyCapability` receba `IPowerSensor` por referência não proprietária e
conduza seu lifecycle. `CompositePowerSensor` preserva o contrato 0.3 ao
consumir snapshots de `IVoltageSensor` e `ICurrentSensor` sem conduzir esses
sensores; `INA3221PowerSensor` lê tensão e corrente do dispositivo compartilhado
e calcula potência em software. O overload anterior permanece compatível e
cria composite pertencente à aplicação. A versão 0.4 está em `Draft` e não
substitui a baseline 0.3 validada enquanto não percorrer análise, implementação,
revisão e decisão do Arquiteto. Exemplos e aplicação MCB01 não são migrados
nesta versão.
...
O encerramento posterior preserva a falha de build, sem novo débito aceito.
...
A versão 0.2 foi encerrada e integrada à `main` por ordem explícita do
Arquiteto: `Active` / `Implemented` / `Done`. O registro final é
...
│   ├── ADC1 0.2: mapas por target, enum NTC e leitura calibrada
...
│   ├── BLE Control V1: canal/dispatcher, Auth e autorização local (0.2, In Progress)
│   └── persistência NVS
...
flowchart LR
    ...
    ADC1["ADC1 0.2: target, enum NTC e milivolts"] -->|"seleção e validação de GPIO"| HW
    ...
    INA -->|"I²C Wire"| HW
    HW -->|"snapshots de tensão e corrente"| PSENSOR["IPowerSensor"]
    ...
    SWIFT["App Swift: segundo toggle BLE"] -.->|"integração Swift pendente: Auth, comando e ACK"| BLE["BluetoothDispatcher — código 0.2"]
...
- `EKOM-CHG-0022`: introduz capacidade configurável de 8 a 12 capabilities,
  snapshot NVS v3 com migração integral de v2 e aplicação MCB01 versionada;
  revisão aderente com limitações de evidência e decisão do Arquiteto promovem
  a versão 0.2 para `Active`/`Validated`; integração sincronizada em `main`
  encerra a entrega como `Done`.
- `EKOM-CHG-0023`: registra `IOTSSC-POWER-ENERGY-CAPABILITY@0.4` com
  `IPowerSensor`, composite compatível, adapter INA3221 calculado em software e
  testes contratados; análise formal `Ready`, implementação não iniciada.
...
### BLE Control V1 — implementação e fronteira
...
A revisão permanece Draft/In Progress/Pending; a análise funcional 0.2 é Ready.
BLE-AN-001 foi descartado por incorporação do contrato de vínculo/revogação.
Fonte do parecer: `docs/reports/2026-09-12T023101Z-0.2-a106d3be-implementability-analysis.md`.
O parecer histórico 0.1 permanece em
`docs/reports/2026-09-12T021149Z-0.1-7b7674aa-implementability-analysis.md`.
A ordem posterior autorizou implementação sob governança local 4.7/4.6,
sem migração para o gate 5.0. Fontes de código:
`src/Core/Commands/BluetoothDispatcher.*`,
`src/Platform/Arduino/Transports/ArduinoBluetoothTransportChannel.*` e
`src/Contracts/Transports/BluetoothControl.h`; composição em `SmartSysApp`.
Builds padrão/habilitado aprovados no ESP32 clássico; aceite operacional e
