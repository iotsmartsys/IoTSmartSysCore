...
#include "mocks/TestTemperatureSensor.h"
...
void test_temperature_builder()
{
    ...
    app::TemperatureSensorConfig cfg;
    cfg.capability_name = "temp_sala";
    cfg.sensor = &sensor;

    auto *cap = builder.addTemperatureSensor(cfg);
    ...
    TEST_ASSERT_NOT_EQUAL_MESSAGE("0", s.value.c_str(), "Temperature should not be zero");
}
...
void setup()
{
    ...
    UNITY_BEGIN();
    RUN_TEST(test_temperature_builder);
    UNITY_END();
}
