...
#include "mocks/TestHeightWaterSensor.h"
...
void test_heightwater_builder()
{
    ...
    TestHeightWaterSensor sensor;
    sensor.setHeightCm(123.45f);

    app::WaterLevelSensorConfig cfg;
    cfg.capability_name = "altura_tanque";
    cfg.sensor = &sensor;

    auto *cap = builder.addWaterHeight(cfg);
    ...
    TEST_ASSERT_NOT_EQUAL_MESSAGE("0", s.value.c_str(), "Height should not be zero");
}
...
void setup()
{
    ...
    UNITY_BEGIN();
    RUN_TEST(test_heightwater_builder);
    UNITY_END();
}
