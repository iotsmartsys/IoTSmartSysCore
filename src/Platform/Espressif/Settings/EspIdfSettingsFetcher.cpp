...
namespace iotsmartsys::platform::espressif
{
    ...
    static Error map_esp_err(esp_err_t e)
    {
        ...
        case ESP_ERR_NO_MEM: return Error::NoMem;
    ...
    iotsmartsys::core::common::Error EspIdfSettingsFetcher::start(const SettingsFetchRequest &req,
                                          ...
                                          void *user_ctx)
    {
        if (!cb || !req.url || req.url[0] == '\0') return Error::InvalidArg;
    ...
    esp_err_t EspIdfSettingsFetcher::performOnce(int &out_http_status)
    {
        ...
        if (!_client) return ESP_ERR_NO_MEM;
    ...
    {
        if (!_body || !data || len <= 0) return true;
    ...
    {
        if (attempt >= _req.max_attempts) return false;
        ...
        // HTTP codes
        if (http_status >= 500 && http_status <= 599) return true;     // servidor
        if (http_status == 408 || http_status == 429) return true;     // timeout / rate limit
        if (http_status >= 400 && http_status <= 499) return _req.retry_on_http_4xx;
